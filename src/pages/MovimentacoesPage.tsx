...
import { Plus, Edit3, TrendingUp, TrendingDown, ChevronUp, Trash2, Check, X } from 'lucide-react';
...
export const MovimentacoesPage: React.FC<MovimentacoesPageProps> = ({
  people,
  categories,
  transactions,
  onSave,
  ...
  }, [editingId, transactions, todayString]);
  ...
  const filteredIncomes = useMemo(() => {
    return transactions
      .filter(t => t.type === 'income' && t.isConfirmed !== false)
      .filter(t => (!startDate || t.date >= startDate))
      .filter(t => (!incomeStartDate || t.date >= incomeStartDate))
      .sort((a, b) => {
        const res = a.date.localeCompare(b.date);
        return incomeSortOrder === 'asc' ? res : -res;
      });
  }, [transactions, startDate, incomeStartDate, incomeSortOrder]);

  const filteredExpenses = useMemo(() => {
    return transactions
      .filter(t => t.type === 'expense' && t.isConfirmed !== false)
      .filter(t => (!startDate || t.date >= startDate))
      .filter(t => (!expenseStartDate || t.date >= expenseStartDate))
      .sort((a, b) => {
        const res = a.date.localeCompare(b.date);
        return expenseSortOrder === 'asc' ? res : -res;
      });
  }, [transactions, startDate, expenseStartDate, expenseSortOrder]);
    ...
    <div className="max-w-6xl mx-auto space-y-10 pb-10">
      <Card ref={formRef} className={`p-8 transition-all duration-300 ${editingId ? 'ring-2 ring-amber-400 bg-amber-50/10 scale-[1.01] shadow-xl' : ''}`}>
        <header className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-[14px] font-black uppercase tracking-widest italic">{editingId ? 'Editar Lançamento' : 'Novo Lançamento'}</h2>
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest block">Lançamentos Confirmados</span>
          </div>
        </header>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-12 gap-x-5 gap-y-4 items-end">
          ...
          <div className="xl:col-span-2 space-y-1.5">
             <Label>Data</Label>
             <Input type="date" required value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} max={todayString} />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Entradas */}
        <section className="card-serious overflow-hidden flex flex-col h-[500px]">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
            <div className="flex items-center gap-2">
              <TrendingUp size={18} className="text-emerald-700" />
              <h3 className="text-[12px] font-bold text-slate-900 uppercase tracking-wider">Entradas</h3>
              <span className="text-[11px] font-bold bg-emerald-100 text-emerald-800 px-2.5 py-1 rounded leading-none">{filteredIncomes.length}</span>
            </div>
                ...
                {filteredIncomes.map(t => (
                  <tr key={t.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-100 group transition-all duration-200 odd:bg-white even:bg-slate-50/50">
                    ...
                    <td className="px-4 py-3 text-right align-top whitespace-nowrap">
                      <span className={`text-[12px] font-bold font-mono tracking-tighter ${editingId === t.id ? 'text-inherit' : 'text-emerald-700'}`}>+{formatCurrency(t.amount)}</span>
                    </td>
                    <td className="px-4 py-3 align-top whitespace-nowrap">
                      <div className="flex gap-2 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => onEdit(t)} className="p-1.5 bg-slate-900 text-white rounded border border-slate-900 hover:bg-white hover:text-slate-900 transition-all"><Edit3 size={12}/></button>
                        <button onClick={() => onDelete(t.id, 'transaction', t.description)} className="p-1.5 bg-slate-900 text-white rounded border border-slate-900 hover:bg-white hover:text-slate-900 transition-all"><Trash2 size={12}/></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="px-5 py-3 border-t border-slate-200 flex justify-between items-center bg-slate-50 shrink-0">
             ...
             <span className="text-sm font-bold text-emerald-800 font-mono tracking-tighter">{formatCurrency(filteredIncomes.reduce((a, b) => a + b.amount, 0))}</span>
          </div>
        </section>
        ...
        <section className="card-serious overflow-hidden flex flex-col h-[500px]">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-[12px] font-bold text-slate-900 uppercase tracking-wider">Saídas</h3>
              <span className="text-[11px] font-bold bg-rose-100 text-rose-800 px-2.5 py-1 rounded leading-none">{filteredExpenses.length}</span>
            </div>
              ...
              <tbody>
                {filteredExpenses.map(t => (
                  <tr key={t.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-100 group transition-all duration-200 odd:bg-white even:bg-slate-50/50">
                    ...
                    <td className="px-4 py-3 align-top min-w-[200px] text-slate-700">
                      <div className="flex flex-col">
                        <span className="text-[12px] font-medium tracking-tight leading-tight">{t.description}</span>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[9px] font-bold uppercase tracking-widest opacity-60 text-slate-400">{categories.find(c => c.id === t.categoryId)?.name}</span>
                    ...
                    <td className="px-4 py-3 text-right align-top whitespace-nowrap">
                      <span className={`text-[12px] font-bold font-mono tracking-tighter ${editingId === t.id ? 'text-inherit' : 'text-rose-700'}`}>-{formatCurrency(t.amount)}</span>
                    </td>
                    <td className="px-4 py-3 align-top whitespace-nowrap">
                      <div className="flex gap-2 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => onEdit(t)} className="p-1.5 bg-slate-900 text-white rounded border border-slate-900 hover:bg-white hover:text-slate-900 transition-all"><Edit3 size={12}/></button>
          ...
          <div className="px-5 py-3 border-t border-slate-200 flex justify-between items-center bg-slate-50 shrink-0">
             <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Total</span>
             <span className="text-sm font-bold text-rose-800 font-mono tracking-tighter">{formatCurrency(filteredExpenses.reduce((a, b) => a + b.amount, 0))}</span>
          </div>
        </section>
      </div>
    </div>
